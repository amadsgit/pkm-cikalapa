...
<head>
    <meta charset="UTF-8">
    <title>Daftar Urut Kepangkatan Pegawai</title>
    <style>
        @page {
...
<body>
    <table>
        <thead>
            ...
            <tr class="no-border">
                <th colspan="16" class="no-border center">
                    <h2 class="title">DAFTAR URUT KEPANGKATAN (DUK) UPTD PUSKESMAS CIKALAPA</h2>
                    <h2 class="title">KABUPATEN SUBANG</h2>
                </th>
            </tr>
            <tr style="background-color: #f8f9fa;">
                <th style="width: 15px; max-width: 15px; font-size: 11px; white-space: nowrap; overflow: hidden;">No
                </th>
        ...
        <tbody>
            @foreach ($dukPegawai as $index => $data)
            <tr>
                <td style="width: 15px; max-width: 15px; font-size: 11px; white-space: nowrap; overflow: hidden;">{{
                    $loop->iteration }}</td>
                <td>{{ $data->nama_pegawai }}</td>
                <td>{{ $data->nip }}</td>
                <td>{{ $data->tempat_lahir }}, {{ \Carbon\Carbon::parse($data->tanggal_lahir)->translatedFormat('d F Y')
                    }}</td>
    ...
    <div class="ttd">
        <div class="right">
            ...
            <br><br><br>
            <p style="text-decoration: underline; font-weight: bold;">{{ optional($kepalaPuskesmas)->nama_pegawai ?? '-'
                }}</p>
