...
<div class="min-h-screen flex flex-col">
    ...
    <main class="flex-grow bg-gradient-to-b from-white to-gray-50">
        <!-- Header Gambar -->
        @php
        // Tentukan kategori dan style (aman saat kategori = null)
        $categorySlug = strtolower($informasi->kategori->slug ?? 'lainnya');
        ...
        $style = $categoryStyles[$categorySlug] ?? [
        ...
        <div class="relative w-full h-[260px] md:h-[380px] lg:h-[420px] overflow-hidden rounded-b-3xl shadow-sm">
            <img src="{{ $informasi->gambar ? asset('storage/' . $informasi->gambar) : '/storage/uploads/berita/default-announcement.jpg' }}"
                alt="{{ $informasi->judul }}"
                class="w-full h-full object-cover brightness-[0.9] hover:brightness-100 transition-all duration-700 ease-out">
            <div class="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent"></div>
        
            <div class="absolute bottom-6 left-6 md:left-10 text-white space-y-2">
                <a href="{{ route('informasi', ['kategori' => $informasi->kategori->slug ?? 'semua']) }}"
                    ...
                    <span>{{ $informasi->kategori->nama ?? 'Lainnya' }}</span>
                </a>
                ...
                <div class="flex items-center gap-3 text-sm text-gray-100/90">
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-10">
                        ...
                        <div class="flex justify-between items-center mb-6 border-b border-gray-100 pb-4">
                            ...
                            <div x-data="{ open: false }" class="relative">
                                ...
                                <div x-show="open" x-transition
                                    ...
                                    <a href="https://wa.me/?text={{ urlencode($informasi->judul . ' - ' . Request::url()) }}"
                                        target="_blank"
                                        class="flex items-center gap-3 px-4 py-2 text-gray-600 hover:bg-gray-50 text-sm transition">
                        ...
                        <div
                            ...
                            <div>
                                <h3 class="text-lg font-semibold text-gray-900">
                                    {{ $informasi->author->pegawai->nama_pegawai ?? 'Admin Puskesmas' }}
                                </h3>
                                <p class="text-sm text-gray-500">Penulis Informasi</p>
                            </div>
                        </div>
                    </article>
                    ...
                    <aside class="lg:col-span-1">
                        @include('landing.components.sidebar-informasi')
                    </aside>
                </div>
            </div>
        </section>
